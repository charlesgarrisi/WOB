...
The imbalance settlement is governed by the positive and negative imbalance prices, denoted $\text{PREP}$ (penalty for surplus) and $\text{PREN}$ (penalty for deficit), respectively. Introducing multi-dimensional uncertainty leads to a bilinear loss, breaking the quantile-based solution.
...
To start with, we use generated data. We proceed incrementally, in the following order.

- 1°) **Stochastic generation only**, i.e. standard Newsvendor, solved with an explicit formula $\leadsto$ see WOB_newsvendor.jl
- 2°) **Stochastic day-ahead prices** 2D uncertainty given by $\xi=(g,S)$ with PREP and PREN modeled as linear offsets of the day-ahead price, e.g., $\text{PREP} = s - \delta_{\text{surplus}}$ and $\text{PREN} = s + \delta_{\text{deficit}}$ $\leadsto$ WOB_stochastic_prices.jl
- 3°) **Stochastic imbalance prices** 4D uncertainty given by $\xi=(g,S,\text{PREP},\text{PREN})$ $\leadsto$ WOB_stochastic_imbal.jl
